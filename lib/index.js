...
geui.version = function () {
    console.log('geui@' + package.version);
};

geui.help = function () {
    console.log('%s @%s', package.description, package.version);
};
...
geui.run = function (config, dir) {
    ...
    if (isLess) {
        var lessWatcher = config.less.options.watcher,
...
function taskUglify(config, dir) {
    ...
    var uglify = config.uglify,
        isLabjs = config.options.isLabjs,
        name = config.name,
        uglifyMap = {};
...
//        labjs依赖替换
        ...
        if (!geui.isEmptyObj(minifyOptions) && !!result.map) {
            ...
            map = path.join(path.dirname(module.dist), map);
